...
export const DeleteConfirmationModal: React.FC<DeleteConfirmationModalProps> = ({
  ...
  const accentClasses = isDangerous
    ? {
        ...
        button:
          'bg-[linear-gradient(135deg,#fb7185_0%,#f43f5e_100%)] text-white shadow-[0_18px_34px_rgba(244,63,94,0.26)] hover:brightness-[1.03]',
      ...
      >
        <div className="admin-hero-card relative overflow-hidden px-6 pb-5 pt-5 text-[#fff8e9]">
          ...
          <div className="flex items-start gap-4">
            <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-[22px] border ${accentClasses.badge}`}>
              {isDangerous ? <AlertTriangle size={24} className={accentClasses.icon} /> : <ShieldAlert size={24} className={accentClasses.icon} />}
            </div>
            <div className="min-w-0 flex-1 pt-0.5">
              <h2 className="text-[24px] font-black leading-tight tracking-tight text-white">{title}</h2>
        ...
        <div className="space-y-5 px-6 pb-6 pt-5">
          {itemName ? (
            <div className="rounded-[22px] border border-[var(--admin-pro-line)] bg-[rgba(255,255,255,0.82)] px-4 py-3 shadow-[0_12px_28px_rgba(74,56,16,0.06)]">
              <p className="text-[11px] font-black uppercase tracking-[0.2em] text-[var(--admin-pro-text-muted)]">Tanlangan obyekt</p>
